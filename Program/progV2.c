...
void outputToFile(char* line, char* outputPath, size_t childNum){
    ...
    // combine output path, command, line, and unique index for correct call to system() 
    ...
    snprintf(redirectedLine, (redirectedLineLen), "%s > %s%s_%d-%02d-%02d-%02d-%02d-%02d-%zd.txt", line, outputPath, command, tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec, childNum);
